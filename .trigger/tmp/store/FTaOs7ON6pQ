...
__export(schema_exports, {
  ...
  dietPreferenceEnum: () => dietPreferenceEnum,
  goalEnum: () => goalEnum,
  languagePreferenceEnum: () => languagePreferenceEnum,
...
// src/db/schema.ts
var unitPreferenceEnum = pgEnum("unit_preference", ["metric", "imperial"]);
var themePreferenceEnum = pgEnum("theme_preference", ["light", "dark", "system"]);
var goalEnum = pgEnum("goal", ["lose", "maintain", "gain"]);
var activityLevelEnum = pgEnum("activity_level", ["sedentary", "light", "moderate", "very", "extra"]);
var dietPreferenceEnum = pgEnum("diet_preference", ["classic", "keto", "vegan", "vegetarian"]);
var mealStatusEnum = pgEnum("meal_status", ["analyzing", "completed", "failed"]);
var languagePreferenceEnum = pgEnum("language_preference", ["en", "id", "system"]);
var timestamps = {
  createdAt: timestamp({ withTimezone: true }).notNull().defaultNow(),
  updatedAt: timestamp({ withTimezone: true }).notNull().defaultNow().$onUpdate(() => /* @__PURE__ */ new Date())
};
var users = pgTable("users", {
  ...
  unitPreference: unitPreferenceEnum(),
  // display only
  themePreference: themePreferenceEnum(),
  // display only, like unitPreference
  gender: text(),
  dateOfBirth: date({ mode: "string" }),
  // string, not Date — a DOB has no timezone
...
var meals = pgTable(
  "meals",
  {
    ...
    triggerRunId: text(),
    // for Realtime subscribe / debugging
...
var db = drizzle({ client: cs(url), schema: schema_exports, casing: "snake_case" });
...
@neondatabase/serverless/index.mjs:
  ...
  *)
*/
//# sourceMappingURL=chunk-S4HBRGEI.mjs.map
